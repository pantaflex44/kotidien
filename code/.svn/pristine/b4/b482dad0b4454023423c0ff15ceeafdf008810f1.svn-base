...
class MdiTrends(MdiFrame):
	...
	def __init__(self, settings, locale, parent, fi: financial, closable: bool = True, title='', ctype='overtime', *args, **kwargs):
		self._title = title.strip()
		self._type = ctype.strip().lower() \
			if ctype.strip().lower() in ['overtime'] \
			else 'overtime'
		super(MdiTrends, self).__init__(settings, locale, parent, fi, closable, *args, **kwargs)
